...
class BroadcastService:
    ...
    async def broadcast_to_users(self, broadcast_id: int, user_ids: list, message_text: str):
        sent = 0
        failed = 0
        blocked = 0
...
async def run_global_broadcast(message_text: str, admin_id: int):
    async with async_session() as session:
        # Create broadcast record
        new_broadcast = Broadcast(sender_id=admin_id, message_data={"text": message_text}, status="processing")
        ...
        asyncio.create_task(service.broadcast_to_users(new_broadcast.id, user_ids, message_text))
