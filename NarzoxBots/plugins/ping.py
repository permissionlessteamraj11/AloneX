...
from NarzoxBots import app, anon, boot, config, lang
from NarzoxBots.helpers import buttons


@Client.on_message(filters.command(["alive", "ping"]) & ~app.bl_users)
@lang.language()
async def _ping(client: Client, m: types.Message):
    start = time.time()
